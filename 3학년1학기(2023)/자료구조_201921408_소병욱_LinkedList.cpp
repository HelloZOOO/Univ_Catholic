...
// ================================= 노드 탐색 함수 ========================================
void PrintNode(int findX, List L) {
    ...
    // 다음 노드가 없을 때까지 출력 
    while (P->Next != NULL) {
        if (P->Next->Element == findX)
        {
            printf("\n%s, %d, %d \n ", P->Next->name, P->Next->Element, P->Next->Status);
            result = 1;
        }
        P = P->Next;
    }
...
int main() {
    ...
    PrintNode(201921408,L); //Print the node that contains your information
    printf("\n======================================================\n");
    cout << endl;;
    cout << endl;;
    cout << "#### 스테이터스 체크 ####" << endl;;
    StatusCheck(1, L); // 스테이터스 출력
    StatusDelete(1, L); // Delete all the nodes with status 1
    cout << endl;;
